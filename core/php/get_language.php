...
//Will try language set in url, in database, in session or default from config.php in this order.
// sets global variables LANG and LANGTYPE.
// returns language object
function get_language($url, $db_lang) {
  ...
  { 
    $repl=array('', '\1 "\2" :');
    $mtch=array('/\/\/.*\n/', '/([{,][ \n]*)\'?(\w+)\'?[ ]*:/');
    return json_decode(preg_replace($mtch, $repl, file_get_contents('language/'.$tla[0].'.js',NULL,NULL,8)), true);
  }
  elseif($tla[1]=='ini') 
  { 
    return parse_ini_file('language/'.$tla[0].'.ini'); 
  }
}
?>
