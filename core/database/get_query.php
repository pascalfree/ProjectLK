...
////////////////////////////////////*/
  ...
  if( $go -> good() ) {
    $query = "SELECT userid,registerid,mode FROM lk_activelist 
              WHERE userid = '".$userid."' AND id = '".$queryid."'";
  ...
  if( $go -> good() ) {
    ...
    $query = "DELETE FROM lk_active 
             ...
             ) AND lk_active.id = '".$queryid."'"; 
    $go -> query( $query,2 );
  }
  //Load / Lade  
  if( $go -> good() ) {   
    //Total 
    $total = $go -> query( "SELECT COUNT(*) FROM lk_active WHERE id = '".$queryid."'",3 );
    $total = $total['result']['COUNT(*)'][0];
  }
  if( $go -> good() ) {
    //Done 
    $done = $go -> query( "SELECT COUNT(*) FROM lk_active WHERE done = '1' AND id = '".$queryid."'",4 ); 
    $done = $done['result']['COUNT(*)'][0];
  }
  if( $go -> good() ) {
    //Correct 
    $correct = $go -> query( "SELECT COUNT(*) FROM lk_active WHERE correct = '1' AND id = '".$queryid."'",5 ); 
  ...
  if( $go -> good() ) {
    ...
    if( $wordid == NULL ) {
      ...
      $wordid = $go -> query( "SELECT wordid FROM lk_active WHERE done = '0' AND id = '".$queryid."' ORDER BY wordid LIMIT ".$getshuffle.",2",6 ); 
      $wordid = $wordid['result']['wordid'];
    }
  }
  if( $go -> good() ) {
    //Wrong Answers
    $wrongword = $go -> query( "SELECT wordid, answer FROM lk_active 
                        WHERE done = '1' AND correct = '0' AND id = '".$queryid."' ORDER BY wordid",7 );
  ...
  if( $go -> good() ) {
    ...
    if( $mode == 4 ) { //Verbquery
      ...
      if( !is_array( $wordid ) ) { $wordid = array($wordid); } //fix : make wordid an array
      $count = count( $wordid );
      for( $i = 0; $i < $count; $i++ ) { 
