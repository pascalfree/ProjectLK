...
$toolbar .= link_generic('action=query&type=verb&id=0&class=icon_text iconplay&icon=play&text='.urlencode($plk_la['query']));
...
if($error) {
  echo $error;
    ...
    list_generic('form', 'addnew=1&title=forms&pluspass=info&registerid='.$plk_here->registerid);
    list_generic('person', 'addnew=1&title=persons&registerid='.$plk_here->registerid);
